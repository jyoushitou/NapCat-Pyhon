...
def get_log_file_name():
    return os.path.join(LOG_DIR, f"bot_log_{datetime.now().strftime('%Y-%m-%d')}.txt")

def write_log_to_file(level: str, msg: str):
    try:
        time_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        with open(get_log_file_name(), "a", encoding="utf-8") as f:
...
# OCR组件（必须保留，因为上面使用了Image和pytesseract）
OCR_ENABLED = False
Image = None
pytesseract = None
...
async def main():
    ...
    log_system("全部初始化完成（AI视觉表情包分析 + 定时随机偏移 + 日志文件已启用）")
    while True:
