        ...
        for item in raw_content:
            ...
            if item_type == "text":
                ...
                if url:
                    ...
                    image_base64, base64_len = encode_image_base64(url)
                    if image_text:
                        image_descriptions.append(f"图片文字识别：{image_text}")
                    if image_base64:
                        if len(image_base64) < base64_len:
                            image_descriptions.append(f"图片Base64(截断)：{image_base64}")
                        else:
                            image_descriptions.append(f"图片Base64：{image_base64}")
                    elif not image_text:
                        image_descriptions.append(f"图片链接：{url}")
                else:
                    image_descriptions.append("图片内容")
            elif item_type == "face":
                face_id = data.get("id")
...
async def get_character_reply(prompt):
    if not API_KEY:
        return random.choice(["真是啰嗦，没必要一直说这些吧","切，随便你怎么想好了","别缠着我，没空胡闹"])
    ...
    req_data = {"model": MODEL_ID, "messages": msg_list, "max_tokens": 128, "temperature": 0.7}
    ...
    for idx in range(MAX_RETRY_TIMES):
        try:
            def req():
                ...
                try:
                    return s.post(API_URL, headers=headers, json=req_data, timeout=API_TIMEOUT, verify=False)
                finally:
                    s.close()
            ...
            if resp.status_code == 200:
                ...
                log_api(f"状态码{resp.status_code}：{resp.text}")
        except requests.exceptions.ReadTimeout:
            log_api(f"第{idx+1}次读取超时")
        except Exception as e:
            log_api(f"第{idx+1}次失败：{str(e)}")
