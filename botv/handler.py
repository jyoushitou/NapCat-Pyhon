# QQ 消息处理主循环
import asyncio, json
from .config import MASTER_QQ, LOCK_RELEASE_DELAY
import botv.config as cfg
from .log import log_recv, log_send, log_err, log_system, log_api
from .image import process_and_save_image, fetch_and_save_acg_image
from .memory import extract_keywords, add_target_memory
...
from .utils import download_url, encode_image_base64, make_image_msg, parse_ai_reply


def contains_sticker_key(text):
    for kw in cfg.STICKER_KEYWORDS:
    ...
    if isinstance(raw_content, list):
        for item in raw_content:
            ...
            if t == "text":
                ...
                log_api(f"[消息解析] 收到图片: {url[:60]}")
                img_data = download_url(url)
                if img_data:
                    md5, tags_str, fp = await process_and_save_image(img_data, url, user_id)
                    log_api(f"[消息解析] 图片处理结果: md5={md5}, tags={tags_str}, fp={fp}")
                    if tags_str:
                        tag_list = tags_str.split(",")
                        img_info.append(f"[{tag_list[0]}]")
                        input_text += " " + " ".join(tag_list[:5])
                else:
                    img_info.append("[图]")
                    log_api(f"[消息解析] 图片下载失败: {url[:60]}")
            elif t == "face":
                fid = str(d.get("id"))
                if fid:
                    cfg.USER_STICKER_CACHE[user_id].append(fid)
                    if len(cfg.USER_STICKER_CACHE[user_id]) > 20:
                        cfg.USER_STICKER_CACHE[user_id].pop(0)
                img_info.append(f"[表情{fid}]")
    else:
        input_text = str(raw_content).strip()
    if img_info:
        input_text = f"{input_text} {' '.join(img_info)}"
    log_api(f"[消息解析] 最终文本: {input_text[:60]}")
    return input_text.strip()


async def websocket_handle_qq(ws):
    async with cfg.active_ws_lock:
        if cfg.active_ws_qq and not getattr(cfg.active_ws_qq,"closed",False): await ws.close(); return
        cfg.active_ws_qq=ws
    ...
    if img_msg and not getattr(ws, "closed", False):
        await send_private_msg(MASTER_QQ, img_msg, ws)
    ...
    try:
        while True:
            ...
            if not mid or mid in cfg.PROCESSED_MSG_IDS: continue
            uid=str(d.get("user_id")); gid=d.get("group_id",0)
            text=await parse_message_content(d.get("message",""),uid)
            if not text: continue
            async with cfg.PROCESS_LOCK:
                cfg.PROCESSED_MSG_IDS.append(mid)
                ...
                if await handle_command(text, uid, ws, mt=="group", gid):
                    ...
                    dialog, action, img_kw, event, refined_kw = parse_ai_reply(ans)
                    add_target_memory(tid,text,ans,img_kw,refined_kw,event)
        ...
        async with cfg.active_ws_lock:
            if cfg.active_ws_qq is ws: cfg.active_ws_qq=None
        cfg.PROCESSED_MSG_IDS.clear(); log_system("QQ断开")
