...
async def call_doubao(msg_list):
    ...
    # 修改：max_tokens 从 128 改为 256，防止截断
...
def split_sentence(text):
    ...
    # 强分隔符（直接分割）
    delimiters = r'[。！？!?；;…]+'
    # 先按强分隔符分割，同时保留分隔符（使用正则来分割并保留分隔符）
    # 方法：使用 re.split 但将分隔符也作为结果的一部分
    ...
    if current.strip():
        sentences.append(current.strip())
    # 如果没有分割出任何句子（比如没有强分隔符），返回原文本
    if not sentences:
        sentences = [text.strip()]
    # 兜底：如果只有一个句子且长度超过50，尝试按逗号、分号等弱分隔符再分割
